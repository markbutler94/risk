...
def gameState(path = 'gamestate.p'):
	pickle.dump([territories, continents, players, cards, cardBonuses, playerList, move, currentPlayer, currentPhase],open(path,'wb'))
...
if loadGamestate = '':
    ...
    with open(loadGamestate, 'rb') as pickle_file:
        territories, continents, players, cards, cardBonuses, playerList, move, currentPlayer, currentPhase = pickle.load(pickle_file)
...
while len(playerList) > 1:
	...
	for p in playerList:
		...
		while tradeIn:
            ...
            cardBonus.append(cardBonus[-1] + 5) # Keep the list of cardBonuses at full length by adding another entry to the end.
